...
namespace TinyExe
{
    ...
    public partial class ParseTreeEvaluator : ParseTree
    {
        ...
        protected override Lazy<object>[] EvalParams(ParseTree tree, params object[] paramlist)
        {
            List<Lazy<object>> parameters = new List<Lazy<object>>();
            for (int i = 0; i < nodes.Count; i += 2)
            {
                if (nodes[i].Token.Type == TokenType.Expression)
                {
                    int pa = i;
                    Lazy<object> val = new Lazy<object>(() => nodes[pa].Eval(tree, paramlist));
                    parameters.Add(val);
                }
            }
        ...
        protected override object EvalFunction(ParseTree tree, params object[] paramlist)
        {
            ...
            Lazy<object>[] parameters = new Lazy<object>[0];
            if (paramNode.Token.Type == TokenType.Params)
                parameters = paramNode.LazyEval(tree, paramlist);
        ...
        protected override object EvalEqualityExpression(ParseTree tree, params object[] paramlist)
        {
            ...
            for (int i = 1; i < nodes.Count; i += 2)
            {
                ...
                if (token.Type == TokenType.EQUAL)
                    ...
                    result = result.ToString().Equals(val.ToString());
        ...
        protected override object EvalExpression(ParseTree tree, params object[] paramlist)
        {
            ...
            if (nodes.Count == 1)
            {
                ...
                try
                {
                    result = this.GetValue(tree, TokenType.AssignmentExpression, 0); // return the result
                }
                catch (Exception ex)
                {
                    tree.Errors.Add(new ParseError(ex.Message, 1, this));
                }
                return result;
            ...
            // ok, this is an assignment so declare the function or variable
                ...
                if (v.Token.Type == TokenType.VARIABLE)
                {
                    ...
                    var value = this.GetValue(tree, TokenType.AssignmentExpression, 1);
                    
                    return root.Context.CurrentScope[key] = value;
