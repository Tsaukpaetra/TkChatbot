...
namespace TinyExe
{
    ...
    public partial class ParseNode
    {
        ...
        internal Lazy<object>[] LazyEval(ParseTree tree, params object[] paramList)
        {
            Lazy<object>[] result = new Lazy<object>[tree.nodes.Count];
            if( Token.Type == TokenType.Params)
                    result = EvalParams(tree, paramList);
            //int index = 0;
            //foreach (ParseTree om in tree.nodes)
            //    result[index++] = new Lazy<object>(() => Eval( om) as object[]);
            return result;
        }
        ...
        protected virtual Lazy<object>[] EvalParams(ParseTree tree, params object[] paramlist)
        {
            throw new NotImplementedException();
        }
